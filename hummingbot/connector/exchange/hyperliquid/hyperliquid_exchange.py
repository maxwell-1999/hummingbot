...
class HyperliquidExchange(ExchangePyBase):
    ...
    def authenticator(self) -> Optional[HyperliquidAuth]:
        if self._trading_required:
            return HyperliquidAuth(
                self.hyperliquid_api_key, self.hyperliquid_secret_key, self._use_vault
            )
        return None
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        if (
            ...
            self.logger().debug(
                f"The order {order_id} does not exist on Hyperliquid s. "
                ...
                if channel == CONSTANTS.USER_ORDERS_ENDPOINT_NAME:
                    ...
                    if "fills" in results:
                        for trade_msg in results["fills"]:
                            ...
                            await self._process_trade_message(
                                trade_msg, client_order_id
                            )
            except asyncio.CancelledError:
                raise
            except Exception:
    ...
    async def _format_trading_rules(
        ...
        self.name_to_coin = {
            asset_info["name"]: asset_info["name"]
            for asset_info in exchange_info_dict[0]["universe"]
        }

        coin_infos: list = exchange_info_dict[0]["universe"]
        price_infos: list = exchange_info_dict[1]

        for spot_info in filter(
            web_utils.is_exchange_information_valid, exchange_info_dict[0]["universe"]
        ):
        ...
        for coin_info, price_info in zip(coin_infos, price_infos):
            ...
            try:
                ...
                step_size = Decimal(
                    str(10 ** -exchange_info_dict[0]["tokens"][base].get("szDecimals"))
                )
                price_size = Decimal(
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: List):
        ...
        self.coin_to_asset = {
            asset_info["name"]: asset
            for (asset, asset_info) in enumerate(exchange_info[0]["universe"])
        }

        self.name_to_coin = {
            asset_info["name"]: asset_info["name"]
            for asset_info in exchange_info[0]["universe"]
        }

        for spot_info in filter(
            web_utils.is_exchange_information_valid, exchange_info[0]["universe"]
        ):
            self.coin_to_asset[spot_info["name"]] = spot_info["index"] + 10000
            self.name_to_coin[spot_info["name"]] = spot_info["name"]
            ...
            if trading_pair in mapping.inverse:
                self._resolve_trading_pair_symbols_duplicate(
                    mapping, name, new_base, new_quote
                )
            else:
    ...
    async def _update_balances(self):
        ...
        account_info = await self._api_post(
            ...
            data={"type": CONSTANTS.USER_STATE_TYPE, "user": self.hyperliquid_api_key},
        )
        balances = account_info["balances"]
        for balance_entry in balances:
            asset_name = balance_entry["coin"]
            free_balance = Decimal(balance_entry["total"]) - Decimal(
                balance_entry["hold"]
            )
                ...
                for trade in trades:
                    ...
                    if exchange_order_id in order_by_exchange_id_map:
                        ...
                        trade_update = TradeUpdate(
                            ...
                            fee=fee,
                            fill_base_amount=Decimal(trade["sz"]),
                            fill_quote_amount=Decimal(trade["sz"])
                            * Decimal(trade["px"]),
                            fill_price=Decimal(trade["px"]),
                            fill_timestamp=trade["time"] * 1e-3,
                        )
                        ...
                        str(trade["tid"]), exchange_order_id, trading_pair
                    ):
                        ...
                        self.trigger_event(
                            ...
                            OrderFilledEvent(
                                ...
                                trade_fee=DeductedFromReturnsTradeFee(
                                    flat_fees=[
                                        TokenAmount(
                                            trade["feeToken"], Decimal(trade["fee"])
                                        )
                                    ]
                                ),
                                exchange_trade_id=str(trade["tid"]),
        ...
        if order.exchange_order_id is not None:
            ...
            for trade in all_fills_response:
                ...
                fee = TradeFeeBase.new_spot_fee(
                    ...
                    flat_fees=[
                        TokenAmount(
                            amount=Decimal(trade["fee"]), token=trade["feeToken"]
                        )
                    ],
                )
                trade_update = TradeUpdate(
