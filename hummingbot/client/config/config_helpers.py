...
class ClientConfigAdapter:
    ...
    def get(self, key: str, default: Any = None) -> Any:
        """Dictionary-like accessor for compatibility with callers expecting a mapping."""
        try:
    ...
    ) -> Generator[ConfigTraversalItem, None, None]:
    ...
    def get_client_data(self, attr_name: str) -> Optional[ClientFieldData]:
        json_schema_extra = (
            self._hb_config.__class__.model_fields[attr_name].json_schema_extra or {}
        )
    ...
    def get_default(self, attr_name: str) -> Any:
        ...
        if isinstance(default, type(Ellipsis)) or isinstance(
            default, PydanticUndefinedType
        ):
            default = None
        return default
    ...
    def decrypt_all_secure_data(self):
        ...
        secure_config_items = (
            ...
            and traversal_item.client_field_data.is_secure
        )
        for traversal_item in secure_config_items:
            ...
            if value == "" or Security.secrets_manager is None:
                ...
                decrypted_value = Security.secrets_manager.decrypt_secret_value(
                    attr=traversal_item.attr, value=value
                )
            *intermediate_items, final_config_element = (
                traversal_item.config_path.split(".")
            )
            config_model = self
    ...
    def _encrypt_secrets(self, conf_dict: Dict[str, Any]):
        ...
        for attr, value in conf_dict.items():
            if isinstance(value, SecretStr):
                ...
                conf_dict[attr] = Security.secrets_manager.encrypt_secret_value(
                    attr, clear_text_value
                )
    ...
    def _decrypt_all_internal_secrets(self):
        ...
        for traversal_item in self.traverse():
            if traversal_item.type_ == SecretStr:
                ...
                decrypted_value = Security.secrets_manager.decrypt_secret_value(
                    traversal_item.attr, encrypted_value
                )
    ...
    def _add_model_fragments(
        ...
        first_level_conf_items_generator = (
            item for item in self.traverse() if item.depth == 0
        )
        ...
        for traversal_item in first_level_conf_items_generator:
            ...
            if attr_comment is not None:
                ...
                attr_comment = "\n".join(
                    f"{comment_prefix}{c}" for c in attr_comment.split("\n")
                )
    ...
    def __setattr__(self, key, value):
        if key == "_hb_config":
            ...
            raise AttributeError(
                "Cannot set an attribute on a read-only client adapter"
            )
...
yaml.add_representer(
    data_type=ClientConfigAdapter,
    representer=client_config_adapter_representer,
    Dumper=SafeDumper,
)
yaml.add_multi_representer(
    data_type=BaseClientModel,
    multi_representer=base_client_model_representer,
    Dumper=SafeDumper,
)
...
def cvar_json_migration(cvar: ConfigVar, cvar_value: Any) -> Any:
    ...
    ) and isinstance(cvar_value, List):
        results = {}
...
def parse_cvar_default_value_prompt(cvar: ConfigVar) -> str:
    ...
    if cvar.default is None:
        ...
        cvar.type == "bool" and isinstance(cvar.prompt, str) and "Yes/No" in cvar.prompt
    ):
    ...
    try:
        ...
        if config_cls is None:  # legacy
            ...
            strategy_module = __import__(
                f"hummingbot.strategy.{strategy}.{cm_key}",
    ...
    try:
        ...
        if pydantic_cm_path.exists():
            ...
            pydantic_cm_mod = __import__(
                f"hummingbot.strategy.{strategy_name}.{pydantic_cm_pkg}",
                fromlist=[f"{pydantic_cm_class_name}"],
            )
            pydantic_cm_class = getattr(pydantic_cm_mod, pydantic_cm_class_name)
    except ImportError:
        logging.getLogger().exception(
            f"Could not import Pydantic configs for {strategy_name}."
        )
    return pydantic_cm_class


async def load_strategy_config_map_from_file(
    yml_path: Path,
) -> Union[ClientConfigAdapter, Dict[str, ConfigVar]]:
...
def api_keys_from_connector_config_map(cm: ClientConfigAdapter) -> Dict[str, str]:
    ...
    for c in cm.traverse():
        if (
            c.value is not None
            and c.client_field_data is not None
            and c.client_field_data.is_connect_key
        ):
            value = (
                c.value.get_secret_value()
                if isinstance(c.value, SecretStr)
                else c.value
            )
...
async def load_yml_into_cm_legacy(
    yml_path: str, template_file_path: str, cm: Dict[str, ConfigVar]
):
    try:
        data = {}
        conf_version = -1
        ...
        for key in template_data:
            ...
            if cvar.is_secure:
                raise DeprecationWarning(
                    "Secure values are no longer supported in legacy configs."
                )
        ...
        logging.getLogger().error(
            "Error loading configs. Your config file may be corrupt. %s" % (e,),
            exc_info=True,
        )
...
async def read_system_configs_from_yml():
    ...
    await load_yml_into_cm_legacy(
        str(TRADE_FEES_CONFIG_PATH),
        str(TEMPLATE_PATH / "conf_fee_overrides_TEMPLATE.yml"),
        fee_overrides_config_map,
    )
...
def save_to_yml_legacy(yml_path: str, cm: Dict[str, ConfigVar]):
    ...
    try:
        ...
        logging.getLogger().error(
            "Error writing configs: %s" % (str(e),), exc_info=True
        )
...
def all_configs_complete(
    strategy_config: Union[ClientConfigAdapter, Dict],
    client_config_map: ClientConfigAdapter,
):
    return (
...
def missing_required_configs_legacy(config_map):
    return [
        ...
        if c.required and c.value is None and not c.is_connect_key
    ]
...
def parse_config_default_to_text(config: ConfigVar) -> str:
    ...
    if config.default is None:
        ...
        config.type == "bool"
        and isinstance(config.prompt, str)
        and "Yes/No" in config.prompt
    ):
