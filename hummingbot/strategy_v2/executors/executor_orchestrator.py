...
from hummingbot.strategy_v2.executors.arbitrage_executor.arbitrage_executor import (
    ArbitrageExecutor,
)
from hummingbot.strategy_v2.executors.data_types import PositionSummary
from hummingbot.strategy_v2.executors.dca_executor.dca_executor import DCAExecutor
from hummingbot.strategy_v2.executors.neutral_grid_executor.neutral_grid_executor import (
    NeutralGridExecutor,
)
...
class PositionHold:
    ...
    def add_orders_from_executor(self, executor: ExecutorInfo):
        ...
        if (
            "held_position_orders" not in custom_info
            or len(custom_info["held_position_orders"]) == 0
        ):
            return
        ...
        for order in custom_info["held_position_orders"]:
            ...
            # Update fees
            self.cum_fees_quote += Decimal(
                str(order.get("cumulative_fee_paid_quote", 0))
            )
    ...
    def get_position_summary(self, mid_price: Decimal):
        ...
        buy_breakeven_price = (
            self.buy_amount_quote / self.buy_amount_base
            if self.buy_amount_base > 0
            else Decimal("0")
        )
        sell_breakeven_price = (
            self.sell_amount_quote / self.sell_amount_base
        ...
        if net_amount_base != 0:
            if is_net_long:
                ...
                remaining_quote = self.buy_amount_quote - (
                    matched_amount_base * buy_breakeven_price
                )
                ...
                remaining_quote = self.sell_amount_quote - (
                    matched_amount_base * sell_breakeven_price
                )
                breakeven_price = remaining_quote / remaining_base
...
class ExecutorOrchestrator:
    ...
    _executor_mapping = {
        ...
        "neutral_grid_executor": NeutralGridExecutor,
    ...
    def _initialize_cached_performance(self):
        ...
        for position in db_positions:
            ...
            if (
                controller_id in self.initial_positions_by_controller
                or controller_id not in self.strategy.controllers
            ):
                continue
            ...
            if (
                ...
                or position.trading_pair
                not in self.strategy.markets.get(position.connector_name, set())
            ):
                self.logger().warning(
    ...
    def _update_cached_performance(
        self, controller_id: str, executor_info: ExecutorInfo
    ):
        """
    ...
    def _load_position_from_db(self, controller_id: str, db_position: Position):
        ...
        position_hold = PositionHold(
            db_position.connector_name, db_position.trading_pair, side
        )
        ...
        if db_position.side == "BUY":
            ...
            position_hold.buy_amount_quote = (
                db_position.amount * db_position.breakeven_price
            )
            ...
            position_hold.sell_amount_quote = (
                db_position.amount * db_position.breakeven_price
            )
    ...
    def _create_initial_positions(self):
        ...
        for (
            controller_id,
            initial_positions,
        ) in self.initial_positions_by_controller.items():
            if controller_id not in self.cached_performance:
                self.cached_performance[controller_id] = PerformanceReport()
                self.active_executors[controller_id] = []
                self.positions_held[controller_id] = []
            ...
            for position_config in initial_positions:
                ...
                if position_config.side == TradeType.BUY:
                    ...
                    position_hold.buy_amount_quote = Decimal(
                        "NaN"
                    )  # Will be calculated lazily
                    position_hold.sell_amount_base = Decimal("0")
                    position_hold.sell_amount_quote = Decimal("0")
                else:
                    position_hold.sell_amount_base = position_config.amount
                    position_hold.sell_amount_quote = Decimal(
                        "NaN"
                    )  # Will be calculated lazily
                    position_hold.buy_amount_base = Decimal("0")
                    position_hold.buy_amount_quote = Decimal("0")
    ...
    def create_executor(self, action: CreateExecutorAction):
        ...
        # MarketsRecorder.get_instance().store_or_update_executor(executor)
        self.logger().debug(
            f"Created {type(executor).__name__} for controller {controller_id}"
        )
    ...
    def stop_executor(self, action: StopExecutorAction):
        ...
        if not executor:
            self.logger().error(
                f"Executor ID {executor_id} not found for controller {controller_id}."
            )
            return
        ...
        for controller_id, executors_list in self.active_executors.items():
            ...
            executors_to_process = [
                ...
                if (
                    ...
                    and executor.executor_info.config.id
                    not in self.executors_ids_position_held
                )
            ]
            ...
            for executor in executors_to_process:
                ...
                existing_position = self._find_existing_position(
                    positions, executor_info, position_side
                )
    ...
    def _determine_position_side(
        ...
        if (
            hasattr(executor_info.config, "position_action")
            and position_mode == PositionMode.HEDGE
        ):
            opposite_side = (
                TradeType.BUY
                if executor_info.config.side == TradeType.SELL
                else TradeType.SELL
            )
            return (
                opposite_side
    ...
    def _find_existing_position(
        self,
        positions: List[PositionHold],
        executor_info: ExecutorInfo,
        position_side: Optional[TradeType],
    ...
    def store_executor(self, action: StoreExecutorAction):
        ...
        try:
            ...
            self.logger().error(
                f"Executor info: {executor.executor_info} | Config: {executor.config}"
            )
    ...
    def get_executors_report(self) -> Dict[str, List[ExecutorInfo]]:
        ...
        for controller_id, executors_list in self.active_executors.items():
            report[controller_id] = [
                executor.executor_info for executor in executors_list if executor
            ]
        return report
    ...
    def get_all_reports(self) -> Dict[str, Dict]:
        ...
        all_controller_ids = set(
            list(self.active_executors.keys())
    ...
    def generate_performance_report(self, controller_id: str) -> PerformanceReport:
        ...
        report.close_type_counts = (
            cached_report.close_type_counts.copy()
            if cached_report.close_type_counts
            else {}
        )
        ...
        for position in positions:
            ...
            report.realized_pnl_quote += (
                position_summary.realized_pnl_quote - position_summary.cum_fees_quote
            )
        ...
        report.global_pnl_quote = (
            report.unrealized_pnl_quote + report.realized_pnl_quote
        )
        report.global_pnl_pct = (
            (report.global_pnl_quote / report.volume_traded) * 100
            if report.volume_traded != 0
            else Decimal(0)
        )

        # Calculate individual PNL percentages
        report.unrealized_pnl_pct = (
            (report.unrealized_pnl_quote / report.volume_traded) * 100
            if report.volume_traded != 0
            else Decimal(0)
        )
        report.realized_pnl_pct = (
            (report.realized_pnl_quote / report.volume_traded) * 100
            if report.volume_traded != 0
            else Decimal(0)
        )
